...
.PHONY: $(BUILDDIR)/test/incfg-utest.xml
endif
...
.PHONY: check
check: $(BUILDDIR)/test/incfg-utest.xml

$(BUILDDIR)/test/incfg-utest.xml: | build-check
	@echo "  CHECK   $(@)"
	$(Q)env LD_LIBRARY_PATH="$(check_lib_search_path)" \
	        $(BUILDDIR)/test/incfg-utest \
	        $(CHECK_VERBOSE) \
	        --xml='$(@)' \
	        run
...
.PHONY: _clean-check
_clean-check:
	$(call rm_recipe,$(BUILDDIR)/test/incfg-utest.xml)
...
else  # ifneq ($(CONFIG_INCFG_UTEST),y)

.PHONY: check
check:

endif # ifeq ($(CONFIG_INCFG_UTEST),y)
